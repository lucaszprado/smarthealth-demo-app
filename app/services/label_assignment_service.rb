class LabelAssignmentService
  def self.create(labelable, label_params)
    label_categories_id_id = [:label_system_id, :label_organ_id, :label_part_id, :label_spacial_group_id, :label_positioning_id]

    ActiveRecord::Base.transaction do
      label_categories_id.each do |category|
        next unless label_params[category].present?
